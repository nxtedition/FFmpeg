...
static int nxt_read_header(AVFormatContext *s)
{
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "nxt: avio_read failed %j\n", ret);
        return ret;        
    }

    if ((nxt->tag & NXT_TAG_MASK) != NXT_TAG) {
        av_log(NULL, AV_LOG_ERROR, "nxt: invalid tag %j\n", nxt->tag);
        return -1;        
    }
    ...
    size = avio_size(bc);
    if (size < 0) {
        av_log(NULL, AV_LOG_ERROR, "nxt: avio_size failed %j\n", size);
        return ret;
    }
    ...
    if (size > 0) {
        ...
        while (step >= NXT_ALIGN) {
            ...
            if (ret < 0) {
                av_log(NULL, AV_LOG_ERROR, "nxt: avio_seek failed %j\n", ret);
                return ret;
            }
    ...
    case DNXHD_115_720p50:
        ...
        return 0;
      default:
        av_log(NULL, AV_LOG_ERROR, "nxt: invalid format %j\n", nxt->format);
        ret = -1;
        goto fail;
    }
fail:
...
static int nxt_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    if (ret < 0) {
        av_log(NULL, AV_LOG_ERROR, "nxt: av_new_packet failed %j\n", ret);
        goto fail;
    }
    ...
    if (ret < size) {
        av_log(NULL, AV_LOG_ERROR, "nxt: avio_read returned unexpected size %j\n", ret);
        ret = -1;
        goto fail;
    }
...
static int64_t nxt_read_seek(AVFormatContext *s, int stream_index, int64_t timestamp, int flags)
{
    ...
    pos = avio_tell(bc) - NXT_ALIGN;
    if (pos < 0) {
        av_log(NULL, AV_LOG_ERROR, "nxt: avio_tell failed %j\n", pos);
    }

...
/**
 * Build the layer A and layer B sequences of source id.
 * @return 0, or AVERROR_BUG if a mask is not primitive.
 */
static inline int ws_sequences(int id, int8_t *seqA, int8_t *seqB)
{
...
static inline int ws_frame_parse(const uint8_t bits[WS_BITS], unsigned *id, unsigned *payload)
{
    ...
    if (v != WS_SYNC)
        return 0;
    if (ws_crc8(bits, WS_BITS - 8) != (uint8_t)((bits[28] << 7) | (bits[29] << 6) |
                                                   (bits[30] << 5) | (bits[31] << 4) |
                                                   (bits[32] << 3) | (bits[33] << 2) |
                                                   (bits[34] << 1) |  bits[35]))
        return 0;
